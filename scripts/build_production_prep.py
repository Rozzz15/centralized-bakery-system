...
if old_workflow in content:
    ...
    print("✅ Updated workflowSteps")
else:
    print("❌ Could not find old workflowSteps")

# ─── Fix 2: Insert Production Prep section ───
# Insert between the end of dashboard section and the start of adv-freemix section
insertion_marker = """  }

  /* ── Advanced Freemix ── */
  if (activeTab === "adv-freemix") {"""

production_prep_section = """  }
...
content = content.replace(insertion_marker, production_prep_section)

if insertion_marker not in content:
    print("❌ Could not find insertion marker for Production Prep section")
else:
    print("✅ Inserted Production Prep section")
...
old_whitelist = 'role === "deco" && ["dashboard", "free-mix", "adv-freemix", "deco-queue", "custom-orders", "inventory", "freezer"].includes(activeTab)'
new_whitelist = 'role === "deco" && ["dashboard", "free-mix", "adv-freemix", "deco-queue", "inventory", "freezer"].includes(activeTab)'

if old_whitelist in app_content:
    app_content = app_content.replace(old_whitelist, new_whitelist)
    print("✅ Removed custom-orders from deco whitelist in App.tsx")
else:
    print("❌ Could not find deco whitelist in App.tsx")
